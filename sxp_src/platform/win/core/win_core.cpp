...
// system_call
//============================================================================
int pfc::system_call(const char *cmd_)
{
  // execute system call
  STARTUPINFO sinfo;
  PROCESS_INFORMATION pinfo;
  mem_zero(&sinfo, sizeof(sinfo));
  mem_zero(&pinfo, sizeof(pinfo));
  sinfo.cb=sizeof(sinfo);
  char *args=str_alloc(cmd_);
  PFC_VERIFY_MSG(CreateProcessA(0, args, 0, 0, FALSE, CREATE_NO_WINDOW, 0, 0, &sinfo, &pinfo), ("Unable to execute command \"%s\"\r\n", cmd_));
  PFC_MEM_FREE(args);
