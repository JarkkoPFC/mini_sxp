...
//============================================================================
win_directory_monitor::win_directory_monitor(const char *filename_, bool notify_subdirs_)
  :m_notify_subdirs(notify_subdirs_)
{
  // setup directory monitoring
  m_dir_handle=CreateFile(filename_, FILE_LIST_DIRECTORY, FILE_SHARE_READ|FILE_SHARE_WRITE|FILE_SHARE_DELETE, 0, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS|FILE_FLAG_OVERLAPPED, 0);
  m_iocp=CreateIoCompletionPort(m_dir_handle, 0, 0, 0);
  PFC_CHECK_MSG(m_iocp, ("CreateIoCompletionPort() call failed\r\n"));
  mem_zero(&m_overlapped, sizeof(m_overlapped));
  PFC_VERIFY_MSG(ReadDirectoryChangesW(m_dir_handle, m_buffer, sizeof(m_buffer), m_notify_subdirs, FILE_NOTIFY_CHANGE_LAST_WRITE|FILE_NOTIFY_CHANGE_CREATION, 0, &m_overlapped, 0),
                 ("ReadDirectoryChangesW() call failed\r\n"));
}
//----

win_directory_monitor::~win_directory_monitor()
{
  // close handles
  CloseHandle(m_iocp);
...
void win_directory_monitor::update(const functor<void(const char *filename_, e_file_notify)> &callback_)
{
  // query queued directory status completions
  PFC_ASSERT(callback_);
  DWORD num_bytes_read=0;
  OVERLAPPED *overlapped=0;
  ULONG_PTR key=0;
  if(GetQueuedCompletionStatus(m_iocp, &num_bytes_read, &key, &overlapped, 0))
  {
    // process all statuses
    const FILE_NOTIFY_INFORMATION *fninfo=(const FILE_NOTIFY_INFORMATION*)m_buffer;
    char filename[1024];
    while(true)
    {
      // parse filename and perform callback
      wcstr_to_cstr(filename, fninfo->FileName, fninfo->FileNameLength/2);
      filename[fninfo->FileNameLength/2]=0;
      switch(fninfo->Action)
      {
        case FILE_ACTION_MODIFIED: callback_(filename, filenotify_write); break;
      }

      // proceed to the next status
      if(!fninfo->NextEntryOffset)
        break;
      fninfo=(const FILE_NOTIFY_INFORMATION*)((const char*)fninfo+fninfo->NextEntryOffset);
    }
    PFC_VERIFY_MSG(ReadDirectoryChangesW(m_dir_handle, m_buffer, sizeof(m_buffer), m_notify_subdirs, FILE_NOTIFY_CHANGE_LAST_WRITE|FILE_NOTIFY_CHANGE_CREATION, 0, &m_overlapped, 0),
                   ("ReadDirectoryChangesW() call failed\r\n"));
  }
...
file_time win_file_system::mod_time(const char *filename_, const char *path_) const
{
  ...
  return file_time(local_time.wYear, uint8_t(local_time.wMonth), uint8_t(local_time.wDay), uint8_t(local_time.wHour), uint8_t(local_time.wMinute), uint8_t(local_time.wSecond));
}
//----
...
owner_ptr<bin_output_stream_base> win_file_system::open_write(const char *filename_, const char *path_, e_file_open_write_mode mode_, uint64_t fpos_, bool makedir_, e_file_open_check fopen_check_)
{
  ...
  LARGE_INTEGER pos={0, 0};
  if(mode_==fopenwritemode_keep && fpos_==uint64_t(-1))
    SetFilePointerEx(f, pos, 0, FILE_END);
  else if(fpos_ && fpos_!=uint64_t(-1))
  {
    pos.QuadPart=fpos_;
...
{
  return PFC_NEW(win_directory_monitor)(dirname_, notify_subdirs_);
}
//----------------------------------------------------------------------------
