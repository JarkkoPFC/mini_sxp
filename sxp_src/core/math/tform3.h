...
template<typename T>
struct tform3
{ PFC_MONO(tform3) {PFC_VAR3(x, y, z);}
...
template<typename T>
struct tform_rt3
{ PFC_MONO(tform_rt3) {PFC_VAR2(rotation, translation);}
  typedef typename math<T>::scalar_t scalar_t;
  // construction
...
template<typename T>
class camera
{ PFC_MONO(camera) {PFC_VAR4(m_view_to_proj, m_view_to_world, m_near, m_far);}
  PFC_POST_LOAD_CALL(update_transforms());
public:
  typedef typename math<T>::scalar_t scalar_t;
  // construction
  camera();
...
{ PFC_MONO(compressed_rotation_spi) {PFC_VAR(m_data);}
public:
  // construction
...
{ PFC_MONO(compressed_rotation_oct) {PFC_VAR(m_data);}
public:
  // construction
...
//============================================================================
struct compressed_rotation_storage_24
{ PFC_MONO(compressed_rotation_storage_24) {PFC_AVAR(m_data, 3);}
  // config
...
//============================================================================
struct compressed_rotation_storage_32
{ PFC_MONO(compressed_rotation_storage_32) {PFC_VAR(m_data);}
  // config
