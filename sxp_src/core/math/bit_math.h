...
template<int> PFC_INLINE uint128_t bit_shift(const uint128_t&);  // shift bits with signed compile-time value: negative values shift to the right and positive to the left
template<int> PFC_INLINE uint64_t bit_shift(uint64_t);
