...
//============================================================================
template<typename T>
PFC_INLINE vec3<T> sample_unit_sphere(const vec3<T> &xi_)
{
  typedef typename math<T>::scalar_t scalar_t;
  scalar_t l=cbrt(xi_.x);
  scalar_t r=l*(scalar_t(1)-xi_.y*xi_.y);
  scalar_t sphi, cphi;
  sincos(sphi, cphi, xi_.z*math<T>::two_pi);
  return vec3<T>(r*cphi, r*sphi, l*xi_.y);
}
//----------------------------------------------------------------------------
