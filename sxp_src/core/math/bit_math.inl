...
{
#ifdef PFC_INTRINSIC_POPCNT64
  uint64_t cnt;
  PFC_INTRINSIC_POPCNT64(cnt, v_);
  return unsigned(cnt);
#else
...
{
#ifdef PFC_INTRINSIC_POPCNT
  uint32_t nb;
  PFC_INTRINSIC_POPCNT(nb, v_);
  return nb;
#else
  // count the number of set bits with SWAR algorithm
  v_-=(v_>>1)&0x55555555;
...
//============================================================================
// Morton code encode/decode
//============================================================================
PFC_INLINE uint16_t encode16_morton2(uint8_t x_, uint8_t y_)
{
#ifdef PFC_INTRINSIC_PDEP32
  uint32_t resx, resy;
  PFC_INTRINSIC_PDEP32(resx, x_, 0x5555);
  PFC_INTRINSIC_PDEP32(resy, y_, 0xaaaa);
  return uint16_t(resx|resy);
#else
...
PFC_INLINE uint32_t encode32_morton2(uint16_t x_, uint16_t y_)
{
#ifdef PFC_INTRINSIC_PDEP32
  uint32_t resx, resy;
  PFC_INTRINSIC_PDEP32(resx, x_, 0x55555555);
  PFC_INTRINSIC_PDEP32(resy, y_, 0xaaaaaaaa);
  return resx|resy;
#else
...
PFC_INLINE void decode16_morton2(uint8_t &x_, uint8_t &y_, uint16_t mc_)
{
#ifdef PFC_INSTINSIC_PEXT32
  uint32_t resx, resy;
  PFC_INSTINSIC_PEXT32(resx, mc_, 0x55555555);
  PFC_INSTINSIC_PEXT32(resy, mc_, 0xaaaaaaaa);
  x_=uint8_t(resx);
  y_=uint8_t(resy);
#else
...
PFC_INLINE void decode32_morton2(uint16_t &x_, uint16_t &y_, uint32_t mc_)
{
#ifdef PFC_INSTINSIC_PEXT32
  uint32_t resx, resy;
  PFC_INSTINSIC_PEXT32(resx, mc_, 0x55555555);
...
//============================================================================
template<int shift>
uint128_t bit_shift(const uint128_t &v_)
{
