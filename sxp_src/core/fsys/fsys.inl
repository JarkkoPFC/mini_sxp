...
file_time::file_time(uint16_t year_, uint8_t month_, uint8_t day_, uint8_t hour_, uint8_t minute_, uint8_t second_)
  :year(year_)
  ,month(month_)
  ,day(day_)
  ,hour(hour_)
...
PFC_INLINE bool operator<(const file_time &ft0_, const file_time &ft1_)
{
  uint64_t t0=(uint64_t((ft0_.year<<8)|ft0_.month)<<32)|uint64_t((ft0_.day<<24)|(ft0_.hour<<16)|(ft0_.minute<<8)|ft0_.second);
  uint64_t t1=(uint64_t((ft1_.year<<8)|ft1_.month)<<32)|uint64_t((ft1_.day<<24)|(ft1_.hour<<16)|(ft1_.minute<<8)|ft1_.second);
  return t0<t1;
}
//----
