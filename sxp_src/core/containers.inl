...
// sarray
//============================================================================
PFC_INTROSPEC_INL_TDEF2(typename T, usize_t cap, sarray<T, cap>)
{
  PFC_CUSTOM_STREAMING(0);
  switch(unsigned(PE::pe_type))
  {
    ...
    {
      // write/display data
      PFC_CHECK_MSG(m_size<=0xffffffff, ("Unable to serialize sarray<%s> that contains more than 2^32-1 elements\r\n", typeid(T).name()));
      uint32_t size=(uint32_t)m_size;
      pe_.var(size, 0, "size");
      pe_.avar((T*)m_data, size, mvarflag_mutable|mvarflag_mutable_ptr);
    } break;
  }
}
//----------------------------------------------------------------------------
...
// list
//============================================================================
PFC_INTROSPEC_INL_TDEF1(typename T, list<T>)
{
  PFC_CUSTOM_STREAMING(0);
  switch(unsigned(PE::pe_type))
  {
    case penum_input:
    {
      ...
      if(is_valid(it))
      {
        unsigned var_flags=0;
        do
        {
          pe_.var(*it, var_flags);
          var_flags=mvarflag_array_tail;
          ++it;
        } while(is_valid(it));
      }
    } break;
    ...
    {
      // write/display data
      PFC_CHECK_MSG(m_size<=0xffffffff, ("Unable to serialize list<%s> that contains more than 2^32-1 elements\r\n", typeid(T).name()));
      uint32_t size=(uint32_t)m_size;
      pe_.var(size, 0, "size");
      list<T>::iterator it=begin();
      unsigned var_flags=mvarflag_mutable|mvarflag_mutable_ptr;
      while(is_valid(it))
      {
        pe_.var(*it, var_flags);
        var_flags|=mvarflag_array_tail;
...
// set
//============================================================================
PFC_INTROSPEC_INL_TDEF2(typename T, class CmpPred, set<T, CmpPred>)
{
  PFC_CUSTOM_STREAMING(0);
  switch(unsigned(PE::pe_type))
  {
    case penum_input:
    {
      ...
      for(uint32_t i=0; i<size; ++i)
      {
        ...
        {
          parent=n;
          parent_childptr=&n->children[m_cmp_pred.before(n->data, v_)];
          n=*parent_childptr;
        }
        ...
        n=p.data;
        PFC_PNEW(&n->data)T;
        pe_.var(n->data, i?mvarflag_array_tail:0);
        n->color=1;
        n->parent=parent;
        n->children[0]=0;
        n->children[1]=0;
    ...
    {
      // write/display data
      PFC_CHECK_MSG(m_size<=0xffffffff, ("Unable to serialize set<%s> that contains more than 2^32-1 elements\r\n", typeid(T).name()));
      uint32_t size=(uint32_t)m_size;
      pe_.var(size, 0, "size");
      typename set<T, CmpPred>::iterator it=begin();
      unsigned var_flags=0;
      while(is_valid(it))
      {
        pe_.var(const_cast<T&>(*it), var_flags);
        var_flags=mvarflag_array_tail;
        ++it;
      }
    } break;
...
// map
//============================================================================
PFC_INTROSPEC_INL_TDEF3(typename K, typename T, class CmpPred, map<K, T, CmpPred>)
{
  PFC_CUSTOM_STREAMING(0);
  switch(unsigned(PE::pe_type))
  {
    ...
    {
      // write/display data
      PFC_CHECK_MSG(m_size<=0xffffffff, ("Unable to serialize map<%s, %s> that contains more than 2^32-1 elements\r\n", typeid(K).name(), typeid(T).name()));
      uint32_t size=(uint32_t)m_size;
      pe_.var(size, 0, "size");
      typename map<K, T, CmpPred>::iterator it=begin();
      unsigned var_flags=0;
      while(is_valid(it))
      {
        pe_.var(*it.m_node, var_flags);
        var_flags=mvarflag_array_tail;
        ++it;
      }
    } break;
...
// hash_set
//============================================================================
PFC_INTROSPEC_INL_TDEF2(typename T, class Config, hash_set<T, Config>)
{
  PFC_CUSTOM_STREAMING(0);
  switch(unsigned(PE::pe_type))
  {
    case penum_input:
    {
      ...
      for(uint32_t i=0; i<size; ++i)
      {
        ...
        // allocate and default construct new item
        eh_data<item> p(*m_allocator, 1, hash_set_traits<T, Config>::block_alloc_align);
        item *it=p.data;
        PFC_PNEW(&it->data)T;

        // read key & value and insert the item to the map
        pe_.var(it->data, i?mvarflag_array_tail:0);
        p.reset();
        item *&prev=m_hash_table[hash_func_t::index(it->data)];
        it->next=prev;
        prev=it;
        ++m_size;
      }
    ...
    {
      // write/display data
      PFC_CHECK_MSG(m_size<=0xffffffff, ("Unable to serialize hash_set<%s> that contains more than 2^32-1 elements\r\n", typeid(T).name()));
      uint32_t size=(uint32_t)m_size;
      pe_.var(size, 0, "size");
      typename hash_set<T>::iterator it=begin();
      unsigned var_flags=0;
      while(is_valid(it))
      {
        pe_.var(const_cast<T&>(*it), var_flags);
...
// hash_map
//============================================================================
PFC_INTROSPEC_INL_TDEF3(typename K, typename T, class Config, hash_map<K, T, Config>)
{
  PFC_CUSTOM_STREAMING(0);
  switch(unsigned(PE::pe_type))
  {
    case penum_input:
    {
      ...
      for(uint32_t i=0; i<size; ++i)
      {
        ...
        // read key & value and insert the item to the map
        pe_.var(*it, i?mvarflag_array_tail:0);
        p.reset();
        item *&prev=m_hash_table[hash_func_t::index(it->key)];
        it->next=prev;
        prev=it;
        ++m_size;
      }
    ...
    {
      // write/display data
      PFC_CHECK_MSG(m_size<=0xffffffff, ("Unable to serialize hash_map<%s, %s> that contains more than 2^32-1 elements\r\n", typeid(K).name(), typeid(T).name()));
      uint32_t size=(uint32_t)m_size;
      pe_.var(size, 0, "size");
      typename hash_map<K, T>::iterator it=begin();
      unsigned var_flags=0;
      while(is_valid(it))
      {
        pe_.var(*it.m_item, var_flags);
        var_flags=mvarflag_array_tail;
        ++it;
      }
    } break;
...
// hash_bimap
//============================================================================
PFC_INTROSPEC_INL_TDEF4(typename K, typename T, class KConfig, class VConfig, hash_bimap<K, T, KConfig, VConfig>)
{
  PFC_CUSTOM_STREAMING(0);
  switch(unsigned(PE::pe_type))
  {
    case penum_input:
    {
      ...
      for(uint32_t i=0; i<size; ++i)
      {
        ...
        // read key & value and insert the item to the bimap
        pe_.var(*it, i?mvarflag_array_tail:0);
        p.reset();
        item *&kprev=m_hash_ktable[hash_kfunc_t::index(it->key)];
        it->next=kprev;
        kprev=it;
        item *&vprev=m_hash_vtable[hash_vfunc_t::index(it->val)];
        it->next_val=vprev;
        vprev=it;
        ++m_size;
      }
    ...
    {
      // write/display data
      PFC_CHECK_MSG(m_size<=0xffffffff, ("Unable to serialize hash_bimap<%s, %s> that contains more than 2^32-1 elements\r\n", typeid(K).name(), typeid(T).name()));
      uint32_t size=(uint32_t)m_size;
      pe_.var(size, 0, "size");
      typename hash_bimap<K, T>::iterator it=begin();
      unsigned var_flags=0;
      while(is_valid(it))
      {
        pe_.var(*it.m_item, var_flags);
...
// str_pool
//============================================================================
void str_pool::swap(str_pool &pool_)
{
  pfc::swap(m_size, pool_.m_size);
  m_strides.swap(pool_.m_strides);
}
//----------------------------------------------------------------------------

usize_t str_pool::size() const
{
  return m_size;
}
//----

str_pool::const_iterator str_pool::begin() const
{
...
//============================================================================
// str_pool::const_iterator
//============================================================================
str_pool::const_iterator::const_iterator()
{
  reset();
}
//----

void str_pool::const_iterator::reset()
{
...
bool str_pool::const_iterator::operator==(const const_iterator &it_) const
{
  return m_string==it_.m_string;
}
//----
...
str_pool::const_iterator &str_pool::const_iterator::operator++()
{
  // advance iterator
  PFC_ASSERT_PEDANTIC(m_string);
  const stride &s=m_pool->m_strides[m_stride_idx];
  m_string+=m_str_size+1;
  if(m_string>=s.data+stride_size-s.free_space)
  {
    // move to the next stride
    if(++m_stride_idx==m_pool->m_strides.size())
    {
      m_string=0;
      m_str_size=0;
      return *this;
    }
    m_string=m_pool->m_strides[m_stride_idx].data;
  }
  m_str_size=pfc::str_size(m_string);
  return *this;
}
//----

const char *str_pool::const_iterator::c_str() const
{
  return m_string;
}
//----

usize_t str_pool::const_iterator::str_size() const
{
  return m_str_size;
}
//----------------------------------------------------------------------------

str_pool::const_iterator::const_iterator(const str_pool &pool_)
  :m_pool(&pool_)
{
  // initialize the iterator
  m_stride_idx=0;
  if(pool_.m_size)
  {
    m_string=pool_.m_strides[0].data;
    m_str_size=pfc::str_size(m_string);
  }
  else
  {
...
PFC_INLINE void swap(str_pool &sp0_, str_pool &sp1_)
{
  sp0_.swap(sp1_);
}
//----------------------------------------------------------------------------
