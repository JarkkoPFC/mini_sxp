...
{
  // check inputs
  PFC_PERF_TIMER_AUTO(radix_sort, "core/sort & search", "radix_sort()");
  typedef typename iterator_trait<Iterator>::value_t value_t;
...
void insertion_sort(Iterator seq_, usize_t num_values_)
{
  ...
  PFC_PERF_TIMER_AUTO(insertion_sort, "core/sort & search", "insertion_sort()");
  typedef typename iterator_trait<Iterator>::value_t value_t;
  if(!num_values_)
    return;
...
{
  // check for input and sort the sequence
  PFC_PERF_TIMER_AUTO(quick_sort, "core/sort & search", "quick_sort()");
...
{
  // check for input and sort the sequence
  PFC_PERF_TIMER_AUTO(merge_sort, "core/sort & search", "merge_sort()");
  PFC_ASSERT_MSG(is_valid(seq_) || !num_values_, ("Destination sequence not defined\r\n"));
  typedef typename iterator_trait<Iterator>::value_t value_t;
...
Iterator linear_search(Iterator seq_, Iterator end_, const U &value_, Predicate pred_)
{
  ...
  PFC_PERF_TIMER_AUTO(linear_search, "core/sort & search", "linear_search()");
  if(seq_==end_)
    return Iterator();
...
{
  // check for empty sequence
  PFC_PERF_TIMER_AUTO(binary_search, "core/sort & search", "binary_search()");
...
{
  // check for empty sequence
  PFC_PERF_TIMER_AUTO(binary_search_lower_bound, "core/sort & search", "binary_search_lower_bound()");
  PFC_ASSERT(!num_values_ || is_valid(seq_));
  if(!num_values_)
    return Iterator();
...
{
  // check for empty sequence
  PFC_PERF_TIMER_AUTO(binary_search_upper_bound, "core/sort & search", "binary_search_upper_bound()");
  PFC_ASSERT(!num_values_ || is_valid(seq_));
  if(!num_values_)
    return Iterator();
...
{
  // check for empty sequence
  PFC_PERF_TIMER_AUTO(find_min, "core/sort & search", "find_min()");
  if(seq_==end_)
    return Iterator();
...
{
  // check for empty sequence
  PFC_PERF_TIMER_AUTO(find_min, "core/sort & search", "find_max()");
  if(seq_==end_)
    return Iterator();
...
{
  // check for empty sequence
  PFC_PERF_TIMER_AUTO(find_max, "core/sort & search", "find_max()");
  if(!num_values_)
    return Iterator();
