...
class hash_set
{ PFC_MONO(hash_set) PFC_INTROSPEC_DECL;
public:
  // nested types
  class const_iterator;
  class iterator;
...
class hash_map
{ PFC_MONO(hash_map) PFC_INTROSPEC_DECL;
public:
  // nested types
  class const_iterator;
  class iterator;
...
private:
  ...
  //==========================================================================
  class item
  { PFC_MONO(item) {PFC_VAR2(key, val);}
  public:
    K key;
    T val;
    item *next;
...
class hash_bimap
{ PFC_MONO(hash_bimap) PFC_INTROSPEC_DECL;
public:
  // nested types
  class const_iterator;
  class iterator;
...
public:
  ...
  void remove_string(const char*);
  void splice(str_pool&);
  //--------------------------------------------------------------------------
...
private:
  ...
  enum {stride_size=1024};
  usize_t m_size;
  array<stride> m_strides;
};
PFC_SET_TYPE_TRAIT(str_pool, is_type_pod_move, true);
//----------------------------------------------------------------------------

//============================================================================
// str_pool::const_iterator
//============================================================================
class str_pool::const_iterator
{
public:
  // construction
  PFC_INLINE const_iterator();
  ...
  // iteration
  PFC_INLINE friend bool is_valid(const const_iterator &it_)  {return it_.m_string!=0;}
  PFC_INLINE bool operator==(const const_iterator&) const;
  PFC_INLINE bool operator!=(const const_iterator&) const;
  PFC_INLINE const_iterator &operator++();
  PFC_INLINE const char *c_str() const;
  PFC_INLINE usize_t str_size() const;
  //--------------------------------------------------------------------------

private:
  friend class str_pool;
  PFC_INLINE const_iterator(const str_pool&);
  //--------------------------------------------------------------------------

  const str_pool *m_pool;
  usize_t m_stride_idx;
  const char *m_string;
  usize_t m_str_size;
};
//----------------------------------------------------------------------------
