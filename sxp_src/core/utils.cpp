...
uint32_t pfc::crc32(const void *p_, usize_t num_bytes_, uint32_t seed_)
{
  // calculate crc-32
  uint32_t crc=seed_;
  const uint8_t *p=reinterpret_cast<const uint8_t*>(p_);
  const uint8_t *end=p+num_bytes_;
  if(num_bytes_)
    do
      crc=(crc>>8)^s_crc32_table[*p^(crc&0xff)];
    while(++p<end);
  return ~crc;
}
//----

uint32_t pfc::crc32(const char *s_, uint32_t seed_)
{
  // calculate crc-32 from the string
  uint32_t crc=seed_;
  const uint8_t *s=reinterpret_cast<const uint8_t*>(s_);
  if(s && *s)
    do
      crc=(crc>>8)^s_crc32_table[*s^(crc&0xff)];
    while(*++s);
  return ~crc;
}
//----------------------------------------------------------------------------
...
// fourcc_id
//============================================================================
uint32_t fourcc_id::filter_id32(uint32_t id32_) const
{
  // filter out invalid FourCC characters from the id
  uint8_t c0=id32_&255, c1=(id32_>>8)&255, c2=(id32_>>16)&255, c3=id32_>>24;
  if(c0<32 || c0>126) c0=' ';
  if(c1<32 || c1>126) c1=' ';
  if(c2<32 || c2>126) c2=' ';
  if(c3<32 || c3>126) c3=' ';
